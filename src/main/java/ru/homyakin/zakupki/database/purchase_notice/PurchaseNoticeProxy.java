...
public class PurchaseNoticeProxy {
    ...
    public void insert(Object parsedObject, Folder folder) {
        if (parsedObject instanceof PurchaseNotice purchaseNotice) {
            purchaseNoticeRepository.insert(purchaseNotice.getBody().getItem().getPurchaseNoticeData(),  folder);
        } else if (parsedObject instanceof PurchaseNoticeAE purchaseNoticeAE) {
            purchaseNoticeRepository.insert(purchaseNoticeAE.getBody().getItem().getPurchaseNoticeAEData(),  folder);
        } else if (parsedObject instanceof PurchaseNoticeAE94FZ purchaseNoticeAE94FZ) {
            purchaseNoticeRepository.insert(purchaseNoticeAE94FZ.getBody().getItem().getPurchaseNoticeAE94FZData(),  folder);
        } else if (parsedObject instanceof PurchaseNoticeAESMBO purchaseNoticeAESMBO) {
            purchaseNoticeRepository.insert(purchaseNoticeAESMBO.getBody().getItem().getPurchaseNoticeAESMBOData(),  folder);
        } else if (parsedObject instanceof PurchaseNoticeEP purchaseNoticeEP) {
            purchaseNoticeRepository.insert(purchaseNoticeEP.getBody().getItem().getPurchaseNoticeEPData(),  folder);
        } else if (parsedObject instanceof PurchaseNoticeKESMBO purchaseNoticeKESMBO) {
            purchaseNoticeRepository.insert(purchaseNoticeKESMBO.getBody().getItem().getPurchaseNoticeKESMBOData(),  folder);
        } else if (parsedObject instanceof PurchaseNoticeOA purchaseNoticeOA) {
            purchaseNoticeRepository.insert(purchaseNoticeOA.getBody().getItem().getPurchaseNoticeOAData(), folder);
        } else if (parsedObject instanceof PurchaseNoticeOK purchaseNoticeOK) {
            purchaseNoticeRepository.insert(purchaseNoticeOK.getBody().getItem().getPurchaseNoticeOKData(), folder);
        } else if (parsedObject instanceof PurchaseNoticeZK purchaseNoticeZK) {
            purchaseNoticeRepository.insert(purchaseNoticeZK.getBody().getItem().getPurchaseNoticeZKData(), folder);
        } else if (parsedObject instanceof PurchaseNoticeZKESMBO purchaseNoticeZKESMBO) {
            purchaseNoticeRepository.insert(purchaseNoticeZKESMBO.getBody().getItem().getPurchaseNoticeZKESMBOData(), folder);
        } else if (parsedObject instanceof PurchaseNoticeZPESMBO purchaseNoticeZPESMBO) {
            purchaseNoticeRepository.insert(purchaseNoticeZPESMBO.getBody().getItem().getPurchaseNoticeZPESMBOData(), folder);
        }
    }
}
